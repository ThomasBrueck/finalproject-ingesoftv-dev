...
class ResolveStatusTest {
    ...
    @Test
    void shouldResolveStatusAndReleaseContacts() {
        String anonymousId = "user-resolve-123";
        ...
        Map<String, Object> phase1Result = Map.of("releasedIds", List.of("contact-l1-1", "contact-l1-2"));
        when(runnableSpec.bind(anyString()).to(anyString())
                .fetch().one())
            .thenReturn(Optional.of(phase1Result));

        Map<String, Object> phase2Result = Map.of("releasedIds", List.of("contact-l2-1"));
        ...
        verify(kafkaTemplate).send(
                ArgumentMatchers.eq("promotion.status.changed"),
    ...
    @Test
    void shouldResolveStatusWithAdminOverride() {
        String anonymousId = "user-resolve-override";
